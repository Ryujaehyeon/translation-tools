...
- 토큰을 마커로 치환(`__ICON_xxx__`, `__DOLLAR_xxx__`, `__Bn__`)한 뒤 번역 후 복원.
  토큰 식별은 `token_parser`의 범위 인식 파서를 쓴다(정규식 사본 없음). 닫힘 없는
  `£word`(원본 모드 오타)도 파서가 인식해 마스킹하며, 복원 시 정상형 `£word£`로 자동
  교정한다. 영어 원문은 보존되므로 `validate`는 이를 `source_unclosed`(style)로만 본다.
- 기본적으로 빈 `korean_value`만 번역
- 하드 토큰 불일치 시 재시도, 재시도 후에도 불일치면 건너뛰고 리포트에 기록
- 번역 후 `auto_patch_tokens()`로 일부 패턴 자동 보정
- 번역으로 변경이 생기면 종료 시 `validate`로 토큰 무결성을 확인하라고 안내한다
