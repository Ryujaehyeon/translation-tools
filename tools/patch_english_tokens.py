...
def get_paths():
    ...
    reports = os.path.normpath(
        os.path.join(base, "..", "maintenance", "reports", "token_validation")
    )
    backups = os.path.normpath(
        os.path.join(base, "..", "maintenance", "backups", "patch_english_tokens")
    )
            ...
            try:
                for row in csv.DictReader(f):
                    ...
                    if not changes:
                        continue
                    rule_ids = "+".join(
                        dict.fromkeys(c[0] for c in changes)
                    )  # 중복 제거, 순서 유지
